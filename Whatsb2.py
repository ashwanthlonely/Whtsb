...
def send_whatsapp(file_path, message):
    ...
    if 'Mobile Number' not in df.columns or 'Feedback' not in df.columns:
        messagebox.showerror("Error", "Excel file must contain 'Mobile Number' and 'Feedback' columns.")
        return
    ...
    for index, row in df.iterrows():
        ...
        time.sleep(15)  # Delay of 15 seconds between each message
        
        if (index + 1) % 100 == 0 and (index + 1) < total_numbers:
            time.sleep(3600)  # Delay of 1 hour after every 100 messages
        
        if (index + 1) % 1000 == 0 and (index + 1) < total_numbers:
            time.sleep(86400)  # Delay of 24 hours after every 1000 messages
...
# Footnote
tk.Label(app, text="Created by Ashwanth", fg="grey").pack(side=tk.BOTTOM, pady=10)
